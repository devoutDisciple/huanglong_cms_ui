...
export default ({ cityDetail, controllerDialog, onSearch, status, editData = {} }) => {
	...
	useEffect(() => {
		if (status === 'new') {
			setState({ title: '新增区/县', satus: 'new' });
		} else {
	...
	return (
		<Modal className={styles.dialog} title={state.title} visible onOk={handleOk} onCancel={handleCancel}>
			<Form form={form} {...formLayout} layout="inline" initialValues={{ city: cityDetail.name }}>
				...
				<Row className={styles.form_row}>
					<FormItem name="name" label="县/区" rules={[{ required: true }]}>
